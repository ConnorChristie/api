...
GRPC_JAVA_PLUGIN_WINDOWS := https://repo1.maven.org/maven2/io/grpc/protoc-gen-grpc-java/1.37.0/protoc-gen-grpc-java-1.37.0-windows-x86_64.exe
GRPC_JAVA_PLUGIN_MACOS := https://repo1.maven.org/maven2/io/grpc/protoc-gen-grpc-java/1.37.0/protoc-gen-grpc-java-1.37.0-osx-x86_64.exe

.PHONY: buf-generate-java
buf-generate-java:
	@rm -rf tools/java && mkdir -p tools/java && \
	pushd tools/java 1>/dev/null && \
		wget $(GRPC_JAVA_PLUGIN_MACOS) -O protoc-gen-grpc-java && \
